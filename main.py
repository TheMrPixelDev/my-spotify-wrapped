...
from helper_functions import read_histories
...
def main() -> None:
    ...
    streams: List[Dict] = read_histories(data_dir=DATA_PATH)
    ...
    # Extracts the data point of every single stream in puts it into separate lists
    ...
    with open(os.path.join(OUTPUT_PATH, "spotify_stats.html"), "w", encoding="UTF-8") as html_file:
        html_file.write(str(final_html_file))
        print(f"Successfully wrote stats to file ({OUTPUT_PATH})")

    with open(os.path.join(OUTPUT_PATH, "style.css"), "w", encoding="UTF-8") as css_file:
        css_file.write(
            """
